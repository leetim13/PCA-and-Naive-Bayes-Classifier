...
def train_mle_estimator(train_images, train_labels):
    ...
    theta = np.matmul(np.transpose(train_images),train_labels) # 784 x 10 array
    sum_labels = train_labels.sum(axis=0) #sum of indicator function
    theta_mle = theta/sum_labels 
    
    N = len(train_images)
    pi_mle = np.ones(N)
    pi_mle = pi_mle.dot(train_labels) * 1/N
...
def train_map_estimator(train_images, train_labels):
    ...
    sum_labels = train_labels.sum(axis=0) #sum of indicator function
    pi_map = sum_labels/N
    theta_map = (theta + 2)/(4 + sum_labels) #using beta_prior
...
def log_likelihood(images, theta, pi):
    ...
    N = len(images)
    shape =(N, N_pi)
    log_like = np.zeros(shape)
    log_p_x = logsumexp(np.log(pi) + np.dot(images, np.log(theta)) + np.dot((1. - images), np.log(1. - theta)), axis=1)
    for c in range(N_pi):
        log_like[:, c] = (np.dot(images, np.log((theta.T)[c])) + np.dot((1. - images), np.log(1. - (theta.T)[c])) 
        - log_p_x) + np.log(pi[c]) #by derivation
    return log_like 
...
def accuracy(log_like, labels):
    ...
    N = len(labels)
    one_of_k = labels[np.arange(N),log_like.argmax(1)]
    accuracy = sum(one_of_k)/N
    return accuracy
...
def image_sampler(theta, pi, num_images):
    ...
    l = len(theta)
    shape = num_images, l
    sampled_images = np.zeros(shape) #new array of given shape and type, filled with zeros.
    for i in range(num_images):
        c = np.random.choice(10, p=pi) #by hint
        sampled_images[i] = np.random.binomial(1, p=theta[:,c])  #by hint
    save_images(sampled_images, "samples.png")
